            ...
            <div class="col-lg-12">
                <div class="row tabelhed d-flex justify-content-between">
                    <div class="col-lg-2 col-md-2 col-sm-2 d-flex">
                            <a class="ad-btn btn text-center" href="{{ route('admin.publishers.create') }}"> Add</a>
                    </div>
                    ...
                    <div class="col-lg-10 col-md-10"> 
                        ...
                        <div class="right-item d-flex justify-content-end" >
                            <form action="" method="GET" class="d-flex">
                                ...
                                <input type="text" name="keyword" id="keyword" class="form-control" value="{{ isset($keyword) ? $keyword : '' }}" placeholder="Search by Name" required>

                                <button class="btn-sm search-btn keyword-btn" type="submit">
                                    <i class="ti-search pl-3" aria-hidden="true"></i>
                                </button>

                                <a href="{{ route('admin.publishers.index') }}" class="btn-sm reload-btn">
                    ...
                    <div class="card-body">
                        <div class="table">
                                ...
                                @if(count($publishers)>0)
                                    ...
                                    @foreach($publishers as $key => $value)
                                        <tr data-entry-id="{{ $value->id }}">
                                            ...
                                            <td>
                                                @if($value->image)
                                                <img src="{{ url(config('app.publisher_image')).'/'.$value->image ?? '' }}" alt=""
                                                    style="height: 60px;border-radius: inherit;width: 87px;">
                                                @else
                                            ...
                                            <span
                                            class="btn btn-xs {{ $value->status ? 'btn-success' : 'btn-danger' }} text-capitalize change-status"
                                            route="{{ route('admin.publishers.change-status', $value->id) }}">{{ $value->status ? 'Active' : 'In-Active' }}</span>
                                            </td>
                                            <td class="text-center">
                                                <!-- <a href="{{ route('admin.publishers.show', $value->id) }}" class="btn btn-sm btn-icon p-1">
                                                    <i class="mdi mdi-eye mx-1" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" title="View"></i>
                                                </a> -->
            
                                                <a href="{{ route('admin.publishers.edit', $value->id) }}" class="btn btn-sm btn-icon p-1">
                                                    <i class="mdi mdi-pencil" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" title="Edit"></i>
                                                </a>

                                                <button type="submit" class="btn btn-sm btn-icon p-1 delete-record" route="{{ route('admin.publishers.destroy', $value->id) }}"><i class="mdi mdi-delete" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" data-bs-html="true" title="Delete"></i></button>
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
                            ...
                            @if ((request()->get('keyword')) || (request()->get('status')) || (request()->get('category')) || (request()->get('items')))
                                {{ $publishers->appends(['keyword' => request()->get('keyword'),'status' => request()->get('status'),'category' => request()->get('category'),'items' => request()->get('items')])->links() }}
                            @else
                                {{ $publishers->links() }}
                            @endif
                        </div>
