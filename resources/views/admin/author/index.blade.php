            ...
            <div class="col-lg-12">
                <div class="row tabelhed d-flex justify-content-between">
                    <div class="col-lg-2 col-md-2 col-sm-2 d-flex">
                            <a class="ad-btn btn text-center" href="{{ route('admin.authors.create') }}"> Add</a>
                    </div>
                    ...
                    <div class="col-lg-10 col-md-10"> 
                        ...
                        <div class="right-item d-flex justify-content-end" >
                            <form action="" method="GET" class="d-flex">
                                ...
                                <a href="{{ route('admin.authors.index') }}" class="btn-sm reload-btn">
                    ...
                    <div class="card-body">
                        <div class="table">
                                ...
                                @if(count($author)>0)
                                    ...
                                    @foreach($author as $key => $value)
                                        <tr data-entry-id="{{ $value->id }}">
                                            ...
                                            <td>
                                                @if($value->image)
                                                <img src="{{ url(config('app.author_image')).'/'.$value->image ?? '' }}" alt=""
                                            ...
                                            route="{{ route('admin.authors.change-status', $value->id) }}">{{ $value->status ? 'Active' : 'In-Active' }}</span>
                                            </td>
                                            <td class="text-center">
                                                ...
                                                <a href="{{ route('admin.authors.edit', $value->id) }}" class="btn btn-sm btn-icon p-1">
                                                    <i class="mdi mdi-pencil" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" title="Edit"></i>
                                                </a>

                                                <button type="submit" class="btn btn-sm btn-icon p-1 delete-record" route="{{ route('admin.authors.destroy', $value->id) }}"><i class="mdi mdi-delete" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" data-bs-html="true" title="Delete"></i></button>
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
                            ...
                            @if ((request()->get('keyword')) || (request()->get('status')) || (request()->get('category')) || (request()->get('items')))
                                {{ $author->appends(['keyword' => request()->get('keyword'),'status' => request()->get('status'),'category' => request()->get('category'),'items' => request()->get('items')])->links() }}
                            @else
                                {{ $author->links() }}
                            @endif
                        </div>
