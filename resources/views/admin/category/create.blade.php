        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.categories.store', 'type='.request()->get('type', '')) }}" method="POST" enctype="multipart/form-data" id="basic-form">
                            @csrf
                          ...
                          <div class="form-group col-md-6">
                            ...
                            <div class="form-group col-md-6  @if(request()->get('type') == 'category') d-none @endif ">
                            <label class="mt-2"> Parent Category <span class="text-danger">*</span></label> 
                                <select name="parent_cat" class="form-control form-select @error('parent_cat') is-invalid @enderror">
                                    <option value="">select parent category</otpion>
                                    @forelse($parant_cal as $val)
                                    <option value="{{$val->id}}" {{ old('parent_cat') ? ((old('parent_cat') == '') ? 'selected' : '' ) : ( (isset($data) && $data->parent_id == $val->id) ? 'selected' : '' ) }}>{{$val->name}}</option>
                                    @empty
                                    No data
                                    @endforelse
                                </select>
